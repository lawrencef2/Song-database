...
class BinarySearchTree:
    ...
    def height(self) -> int:
        ...
        if self.is_empty():
            return 0
        elif self._left == BinarySearchTree(None) and self._right == BinarySearchTree(None):
            return 1
        else:
    ...
    def items_in_range(self, start: Any, end: Any) -> list:
        ...
        if self.is_empty():
            ...
            final_list = []
            final_list.extend(self._left.items_in_range(start, end))
            if start <= self._root <= end:
                final_list.append(self._root)
            final_list.extend(self._right.items_in_range(start, end))
            return final_list
