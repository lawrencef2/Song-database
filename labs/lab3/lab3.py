...
class Player:
    """
    Generic player class, to be inherited
    """
    ...
    def __str__(self):
        return f'{self.name} has {str(self.wins)} wins'
...
class RandomPlayer(Player):
    """
    AI controlled player that moves by a random amount between min and max step
    """

    def move(self, current: int, min_step: int, max_step: int, goal:int) -> int:
        """
        Return a random integer between min_step and max_step
        """
...
class UserPlayer(Player):
    """
    User controlled player, that moves by how much the user wants
    """
    def move(self, current: int, min_step: int, max_step: int, goal:int) -> int:
        """
        Ask the player for a move amount between min_step and max_step,
        then return that amount.
        If the player enters an invalid input, prompt them
        to enter again.
        """
        move = int(input("How much do you want to move by?:"))
        while move < min_step or move > max_step:
            print(f'Error: Your move must be between {min_step} and {max_step}')
            move = int(input("Please enter a valid input:"))
        return move


class StrategicPlayer(Player):
    """
    AI controlled player that moves optimally
    If there is no winning move, it moves by min_step
    """

    def move(self, current: int, min_step: int, max_step: int, goal:int) -> int:
        """
        Use an algorithm to calculate the winning move from current position,
        then execute that move.
        If no winning move, move by random amount
        """

        diff_from_winning_position = (goal - current) % (max_step + min_step)
        if diff_from_winning_position >= min_step:
            return diff_from_winning_position
        else:
            return random.randint(min_step, max_step)
...
def make_player(generic_name: str) -> Player:
    ...
    Creates a random player by default
    """
...
def main() -> None:
    ...
    p1 = make_player('UserPlayer')
    p2 = make_player('StrategicPlayer')
    while True:
