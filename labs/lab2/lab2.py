...
class Tweet:
    ...
    def retweet(self, new_user: str, new_date: date) -> Tweet:
        ...
        >>> t2 = t1.retweet('Diane', date(2017, 8, 20))
        >>> t2.content
        'David is so cool!'
        >>> t2.userid
        'Diane'
        ...
        """
        return Tweet(new_user, new_date, self.content)
...
class User:
    ...
    def verbosity(self, y: int) -> int:
        ...
        0
        """
        current_year_characters = 0
        for post in self.tweets:
            if post.created_at.year == y:
                current_year_characters += len(post.content)
        return current_year_characters
